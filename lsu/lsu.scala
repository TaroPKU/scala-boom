  ...
  with rocket.HasL1HellaCacheParameters
{
 ...
 // Prioritize emptying the store queue when it is almost full
  ...
  val stq_paddr_mask = (VecInit.tabulate(numStqEntries) { i => 
                        stq(i).valid && stq(i).bits.addr.valid && !stq(i).bits.addr_is_virtual}).asUInt //xq
  ...
  for (w <- 0 until memWidth) {
    ...
    {
      val ldq_idx = exe_req(w).bits.uop.ldq_idx
  ...
  for (i <- 0 until numLdqEntries) {
    ...
    val mask_overlap = widthMap(w => (l_mask & lcam_mask(w)).orR) //.oR检查位向量中是否有至少一位为真（1） 检查是否有重叠部分
    ...
    for (w <- 0 until memWidth) {
      ...
      when (do_release_search(w) &&
        ...
        ldq(i).bits.observed := true.B
                   ...
                   mask_overlap(w) //&&!l_bits.mdp_wait && false.B
                   ) {//xq
        ...
        when (searcher_is_older) {//fired_load更老
        ...
        } .elsewhen (lcam_ldq_idx(w) =/= i.U) { //fired_load更新
